...
from deepgrass.data_util import SSMDataset
import json
import logging
import numpy as np
...
class SSM:
    ...
    def simulate_with_data(self, valid_data):
        config = self.config
        validset = SSMDataset(valid_data, train=False)
        ...
        for i, batch in enumerate(validloader, 0):
            ...
            loss, loss_dict, state_generated, obs_generated = self._simulate_with_batch(batch)
    ...
    def fit(self, train_data, valid_data):
        config = self.config
        batch_size = config["batch_size"]
        trainset = SSMDataset(train_data, train=True)
        validset = SSMDataset(valid_data, train=False)
